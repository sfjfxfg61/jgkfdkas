...
def default_market(lang: str, ref: str = "") -> str:
    prefix = (ref or "").lower().split("_", 1)[0]
    if prefix in MARKETS:
        return prefix
    return {
        "uk": "ua",
        "ru": "cis",
...
def paywall_text(lang: str, market: str) -> str:
    ...
    if lang == "ru":
        return (
            f"<b>Выбери уровень {settings.companion_name}</b>\n"
            f"Регион цен: <b>{MARKET_NAMES[market]}</b>\n\n"
    ...
    if lang == "uk":
        return (
            f"<b>Обери рівень {settings.companion_name}</b>\n"
            f"Регіон цін: <b>{MARKET_NAMES[market]}</b>\n\n"
    ...
    if lang == "es":
        return (
            f"<b>Elige tu experiencia con {settings.companion_name}</b>\nRegión de precios: <b>{MARKET_NAMES[market]}</b>\n\n"
    ...
    if lang == "de":
        return (
            f"<b>Wähle dein {settings.companion_name}-Erlebnis</b>\nPreisregion: <b>{MARKET_NAMES[market]}</b>\n\n"
    ...
    if lang == "fr":
        return (
            f"<b>Choisis ton expérience {settings.companion_name}</b>\nRégion tarifaire : <b>{MARKET_NAMES[market]}</b>\n\n"
    ...
    return (
        f"<b>Choose your {settings.companion_name} experience</b>\n"
        f"Pricing region: <b>{MARKET_NAMES[market]}</b>\n\n"
