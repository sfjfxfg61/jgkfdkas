...
def _channel_ref() -> int | str | None:
    """Accept the existing Render key as a channel ID or public username/link."""
    raw = (os.getenv("PRIVATE_CHANNEL_ID") or os.getenv("PRIVATE_CHANNEL_URL") or "").strip()
    if not raw:
        return None
    ...
    if parsed.hostname in {"t.me", "telegram.me"}:
        ...
        if path.startswith("c/") and path.split("/")[1].isdigit():
            return int("-100" + path.split("/")[1])
        if path and "/" not in path and path.replace("_", "").isalnum():
            return "@" + path
    return None
...
class Settings:
    ...
    pro_ai_model: str = os.getenv("PRO_AI_MODEL") or os.getenv("GROQ_MODEL", "openai/gpt-oss-20b")
    ultra_ai_model: str = os.getenv("ULTRA_AI_MODEL") or os.getenv("GROQ_MODEL", "openai/gpt-oss-20b")
    ...
    private_channel_ref: int | str | None = _channel_ref()
