...
async def start(message: Message, command: CommandObject) -> None:
    ...
    if user.get("onboarding_complete"):
        ...
        return
    safe_name = html.escape(message.from_user.first_name or "there")
    await message.answer(
        t(lang, "intro", name=safe_name),
        reply_markup=keyboards.intro_kb(lang, settings.public_channel(lang)),
    )
...
async def onboarding_begin(callback: CallbackQuery) -> None:
    _, lang = await _user(callback)
    await callback.answer()
    await _edit(callback, t(lang, "choose_style"), keyboards.styles_kb(lang))


@router.callback_query(F.data.startswith("style:"))
async def choose_style(callback: CallbackQuery) -> None:
    style = callback.data.split(":", 1)[1]
    if style not in {"warm", "playful", "calm"}:
        return await callback.answer("Invalid style", show_alert=True)
    user = await store.update_user(callback.from_user.id, style=style, onboarding_complete=True)
    lang = user["lang"]
    await store.track_event(callback.from_user.id, "onboarding_completed", {"style": style})
    await callback.answer()
    await _edit(callback, t(lang, "ready", style=t(lang, f"style_{style}").lower()), keyboards.main_kb(lang))
...
async def nav_chat(callback: CallbackQuery) -> None:
    _, lang = await _user(callback)
    await callback.answer()
    await _edit(callback, t(lang, "chat_prompt"), keyboards.main_kb(lang))
...
async def nav_profile(callback: CallbackQuery) -> None:
    ...
    used = user.get("daily_message_count", 0) if str(user.get("daily_message_date")) == date.today().isoformat() else 0
    remaining = max(limits.daily_messages - int(used), 0)
    level, _ = relationship_level(int(user.get("xp", 0)))
    text = t(
        lang,
        "profile_text",
        style=t(lang, f"style_{user.get('style', 'warm')}").lower(),
        level=f"{level}/4",
        messages=user.get("total_messages", 0),
        plan=tier.title(),
        remaining=remaining,
    )
    text += f"\nPricing: {MARKET_NAMES.get(user.get('market', 'global'), 'Global')}"
    await callback.answer()
    await _edit(callback, text, keyboards.main_kb(lang))


@router.callback_query(F.data == "nav:memory")
async def nav_memory(callback: CallbackQuery) -> None:
    user, lang = await _user(callback)
    limits = entitlements(_tier(user))
    items = await store.memories(callback.from_user.id, limits.memories)
    if not items:
        text = t(lang, "memories_empty")
    else:
        lines = [f"• {html.escape(str(item['memory_value']))}" for item in items[:30]]
        text = t(lang, "memories_title", items="\n".join(lines))
    await callback.answer()
    await _edit(callback, text, keyboards.main_kb(lang))


@router.callback_query(F.data.in_({"nav:settings", "settings:proactive"}))
async def nav_settings(callback: CallbackQuery) -> None:
    user, lang = await _user(callback)
    if not user:
        return await callback.answer("/start", show_alert=True)
    if callback.data == "settings:proactive":
        if not entitlements(_tier(user)).proactive:
            await callback.answer("Available on Plus and higher", show_alert=True)
            return await show_paywall(callback)
        user = await store.update_user(callback.from_user.id, proactive_enabled=not bool(user.get("proactive_enabled")))
    state = t(lang, "on" if user.get("proactive_enabled") else "off")
    text = t(lang, "settings_text", proactive=state, style=t(lang, f"style_{user.get('style', 'warm')}").lower())
    text += f"\nPricing: {MARKET_NAMES.get(user.get('market', 'global'), 'Global')}"
    await callback.answer()
    await _edit(callback, text, keyboards.settings_kb(lang, bool(user.get("proactive_enabled"))))
...
@router.callback_query(F.data == "market:choose")
async def market_choose(callback: CallbackQuery) -> None:
    await callback.answer()
    await _edit(callback, "Choose the region where you normally live:", keyboards.market_kb())


@router.callback_query(F.data.startswith("market:set:"))
async def market_set(callback: CallbackQuery) -> None:
    market = callback.data.rsplit(":", 1)[1]
    if market not in MARKETS:
        return await callback.answer("Invalid region", show_alert=True)
    user = await store.update_user(callback.from_user.id, market=market)
    await store.track_event(callback.from_user.id, "market_changed", {"market": market})
    await callback.answer(f"Pricing region: {MARKET_NAMES[market]}")
    await _edit(callback, paywall_text(user["lang"], market), keyboards.premium_kb(user["lang"], market, bool(user.get("subscription_recurring"))))


@router.callback_query(F.data == "memory:confirm")
async def memory_confirm(callback: CallbackQuery) -> None:
    _, lang = await _user(callback)
    await callback.answer()
    await _edit(callback, t(lang, "clear_confirm"), keyboards.confirm_clear_kb(lang))


@router.callback_query(F.data == "memory:clear")
async def memory_clear(callback: CallbackQuery) -> None:
    _, lang = await _user(callback)
    await store.clear_user_memory(callback.from_user.id)
    await store.track_event(callback.from_user.id, "memory_cleared")
    await callback.answer()
    await _edit(callback, t(lang, "cleared"), keyboards.main_kb(lang))
...
async def show_paywall(callback: CallbackQuery) -> None:
    ...
    if not user:
        return await callback.answer("/start", show_alert=True)
    market = user.get("market") or default_market(lang)
...
async def create_checkout(callback: CallbackQuery, bot: Bot) -> None:
    ...
    kwargs = {
        "title": f"{settings.companion_name} {product.title}"[:32],
        "description": (
            f"{product.title}: {entitlements(product.tier).daily_messages} messages/day, "
            f"expanded memory and plan-specific AI model."
        )[:255],
        "payload": payload,
        "provider_token": "",
        "currency": "XTR",
...
async def admin_stats(message: Message) -> None:
    ...
    await message.answer(
        "<b>AI Companion · business stats</b>\n\n"
        f"Users: <b>{stats.get('users', 0)}</b>\n"
        f"Active 7d: <b>{stats.get('active_7d', 0)}</b>\n"
        f"Paying: <b>{stats.get('premium', 0)}</b>\n"
        f"Payment conversion: <b>{stats.get('conversion_pct', 0)}%</b>\n"
        f"Messages: <b>{stats.get('messages', 0)}</b>\n"
        f"Revenue: <b>{stats.get('revenue_stars', 0)} ⭐</b>\n"
        f"ARPPU: <b>{stats.get('arppu_stars', 0)} ⭐</b>\n"
        f"By tier: <code>{html.escape(str(stats.get('tiers', {})))}</code>\n"
        f"By market: <code>{html.escape(str(stats.get('markets', {})))}</code>\n"
        f"By variant: <code>{html.escape(str(stats.get('variants', {})))}</code>"
    )
        ...
        try:
            ...
            if not quota.get("allowed"):
                ...
                return await message.answer(
                    ...
                    reply_markup=keyboards.premium_kb(lang, user.get("market", "global"), bool(user.get("subscription_recurring"))),
                )
            ...
            await message.answer(html.escape(reply), reply_markup=keyboards.main_kb(lang))
